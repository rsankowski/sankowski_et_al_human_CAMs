...
#Load datasets(
if (!file.exists(file.path("data","Index_anon","index_all_gbm.RData"))) {
        #plate layout
        ...
        bcd_plate2$cell_ID <- ifelse(bcd_plate2$Y<13,paste0(sample_name, '_5_', bcd_plate2$Barcode),paste0(sample_name, '_6_', bcd_plate2$Barcode))
        bcd_plate2 <- bcd_plate2 %>% left_join(metadata[,!grepl("(Pat_ID|_ind)$", colnames(metadata))])
        ...
        #define compartment since the index files have not worked
        index_full$Compartment_ind <- ifelse(index_full$Y > 6 & index_full$Y < 13 & index_full$X %in% LETTERS[2:16], "MenM", "Dura")
        index_full$Diagnosis_ind <- "Ctrl"
        index_list[[sample_name]] <- index_full[index_full$cell_ID %in% rownames(metadata),]
        ...
        index2 <- read_csv2("data/Index_anon/Pat20_GM_WM_PVM_3.csv") %>%
                add_column(Compartment_ind = "PVM", Diagnosis_ind = "Ctrl", Pat_ID = "Pat20", Region_ind = "WM")
        ...
        #Pat6_11_GBM_Men
        bcd_plate2 <- bcd_plate
        sample_name <- 'Pat6_Pat11_GBM_Men'
        bcd_plate2$cell_ID <- ifelse(bcd_plate2$Y <13, paste0(sample_name, '_1_', bcd_plate2$Barcode), paste0(sample_name, '_', bcd_plate2$Barcode))
        bcd_plate2 <- bcd_plate2 %>% left_join(metadata[,!grepl("(Pat_ID|_ind)$", colnames(metadata))])
        
        #load index data
        index1 <- read_csv("data/Index_anon/Pat6_Pat11_GBM_Men.csv") %>%
                add_column(Compartment_ind = "MenM", Diagnosis_ind = "GBM", Pat_ID = "Pat6", Region_ind = "Other")
        
        index <- read_csv("data/Index_anon/Pat6_Pat11_GBM_Men_2.csv") %>%
        ...
        bcd_plate2$cell_ID <- ifelse(bcd_plate2$Y <13, paste0(sample_name, '_', bcd_plate2$Barcode), paste0(sample_name, '_8_', bcd_plate2$Barcode))
        bcd_plate2 <- bcd_plate2 %>% left_join(metadata[,!grepl("(Pat_ID|_ind)$", colnames(metadata))])
        ...
        bcd_plate2$cell_ID <- ifelse(bcd_plate2$Y <13, paste0(sample_name, '_4_', bcd_plate2$Barcode), paste0(sample_name, '_', bcd_plate2$Barcode))
        bcd_plate2 <- bcd_plate2 %>% left_join(metadata[,!grepl("(Pat_ID|_ind)$", colnames(metadata))])
        ...
        bcd_plate2$cell_ID <- ifelse(bcd_plate2$Y <13, paste0(sample_name, '_7_', bcd_plate2$Barcode), paste0(sample_name, '_', bcd_plate2$Barcode))
        bcd_plate2 <- bcd_plate2 %>% left_join(metadata[,!grepl("(Pat_ID|_ind)$", colnames(metadata))])
        ...
        save(index_all, file = file.path("data","Index_anon","index_all_gbm.RData"))
} else {
...
for (i in colnames(index_all)[20:29]) {
        ...
        print(pl)
        ggsave(file.path("plots","umap","celseq_gbm",paste0(i,"-logscale.pdf")), useDingbats=F)
}

for (i in colnames(index_all)[20:29]) {
        pl <- plot_index(gene=c(i), point_size = 4, log=F)  + scale_color_gradientn(colors=my_cols) + theme(text=element_text(size=20))
        print(pl)
        ggsave(file.path("plots","umap","celseq_gbm",paste0(i,"-linear-scale.pdf")), useDingbats=F)
}
...
ggsave(file.path("plots","others","celseq_gbm","gene-violin-plots-linear-scale.pdf"), height = 7, width = 7,useDingbats=F)
...
ggsave(file.path("plots","others","celseq_gbm","gene-violin-plots-log-scale.pdf"), height = 7, width = 7,useDingbats=F)
...
for (i in colnames(index_all)[21:26]) {
        ...
        print(pl)
        ggsave(file.path("plots","others","celseq_gbm",paste0(i,"-gene-violin-plots-log-scale.pdf")), useDingbats=F)
...
for (i in colnames(index_all)[21:26]) {
        ...
        print(pl)
        ggsave(file.path("plots","others","celseq_gbm",paste0(i,"-gene-violin-plots-linear-scale.pdf")), useDingbats=F)
