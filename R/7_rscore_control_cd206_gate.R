...
#exclude GBM samples
  prdata <- prdata[, !grepl("GBM", colnames(prdata))]
  ...
  #score for low quality cells
  #export gene names
  write.csv(tibble(ID=rownames(all[["RNA"]]@data)[rowSums(all[["RNA"]]@data)>0]),file.path("data","analysis_output_CD206_gate","genes_celseq2_data.csv"))
  
  #save harmony integration
  save(all, file = file.path("data","SCORE-integration.RData"))
